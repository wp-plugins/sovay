 ...
 */
function sovay_activate_plugin() {
	global $wp_rewrite;

	// if first time activation, set OpenID capability for administrators
	if (get_option('sovay_plugin_revision') === false) {
		global $wp_roles;
	...
	add_option( 'sovay_plugin_revision', 0 );
	add_option( 'sovay_db_revision', 0 );
	...
	update_option( 'sovay_plugin_revision', SOVAY_PLUGIN_REVISION );
 ...
 */
function sovay_deactivate_plugin() {
	wp_clear_scheduled_hook('cleanup_openid');
 ...
 */
function sovay_uninstall_plugin() {
	...
	delete_option('sovay_plugin_revision');
	delete_option('sovay_db_revision');
...
function openid_js_setup() {
	if (have_comments() || comments_open() || is_admin()) {
		wp_enqueue_script('openid', plugin_dir_url(__FILE__) . 'f/openid.js', array('jquery'), SOVAY_PLUGIN_REVISION);
	}
}
...
function openid_style() {
	if ( !wp_style_is('openid', 'registered') ) {
		wp_register_style('openid', plugin_dir_url(__FILE__) . 'f/openid.css', array(), SOVAY_PLUGIN_REVISION);
	}
