...
function openid_admin_panels() {
	...
	// global options page
//	$hookname = add_options_page(__('Sovay options', 'openid'), __('Sovay', 'openid'), 'manage_options', 'openid', 'openid_options_page' );
...
//	todo: will implement controls for forceauth level on login
	...
	if ($user->has_cap('use_openid_provider')) {
        //km removed - we are not providing delegation
//		add_action('show_user_profile', 'openid_extend_profile', 5);
	...
	if ( function_exists('is_site_admin') ) {
		...
		if ( get_option('users_can_register') ) {
			add_settings_field('openid_general_settings', __('Sovay Settings', 'openid'), 'openid_general_settings',
				'general', 'default');
		}
	}

	// add OpenID options to Discussion Settings page
	add_settings_field('openid_disucssion_settings', __('Sovay Settings', 'openid'), 'openid_discussion_settings', 'discussion', 'default');
}
...
function openid_options_page() {
	...
	<div class="wrap">
		<form method="post" action="options.php">
			...
			<h2><?php _e('Sovay Settings', 'openid') ?></h2>

			<div class="updated fade"><p><?php _e('Please note that all Sovay Consumer options have been moved to their respective sections of the '
			...
			<p><?php _e('Sovay allows authorized users to delegate their authentication to Sovay.', 'openid'); ?></p>

			<table class="form-table optiontable editform">
				<tr valign="top">
					<th scope="row"><?php _e('Enable Sovay', 'openid') ?></th>
					<td>

						<p><?php _e('Enable the local Sovay Provider for these roles:', 'openid'); ?></p>
			...
			<table class="form-table optiontable editform">
				<tr valign="top">
					...
					<td>
						...
						<p><?php printf(__('If users are experiencing problems logging in with Sovay, it may help to %1$srefresh the cache%2$s.', 'openid'),
...
function openid_manage_trusted_sites() {
	...
	<div class="wrap">
		...
		<p><?php _e('This is a list of sites that you can automatically login to using your Sovay account.  '
			. 'You will not be asked to approve Sovay login requests for your trusted sites.' , 'openid'); ?></p>
...
function openid_printSystemStatus() {
	...
	$status[] = array( 'Plugin Revision', 'info', SOVAY_PLUGIN_REVISION);
	$status[] = array( 'Plugin Database Revision', 'info', get_option('sovay_db_revision'));
...
function openid_profile_management() {
	...
	switch( $action ) {
		case 'add':
			...
			$sovayid = $_POST['openid_identifier'];
			if (strpos($sovayid, '/') === false) {
				$sovayid = "https://sovay.com/" . $sovayid;
			}
			openid_start_login($sovayid, 'verify', $finish_url);
			break;
		...
		default:
			if ( array_key_exists('message', $_REQUEST) ) {
				...
				$messages = array(
					'',
					__('Unable to authenticate with Sovay.', 'openid'),
					__('Sovay assertion successful, but this URL is already associated with another user on this blog.', 'openid'),
					__('Added association with Sovay.', 'openid')
				);
...
function openid_profile_delete_openids($delete) {
	...
	if (sizeof($urls) == sizeof($delete) && !@$_REQUEST['confirm']) {
		$html = '
			<h1>'.__('Sovay Warning', 'openid').'</h1>
			<form action='.sprintf('%s?page=%s', $_SERVER['PHP_SELF'], $_REQUEST['page']).' method="post">
			<p>'.__('Are you sure you want to delete all of your Sovay associations? Doing so may prevent you from logging in.', 'openid').'</p>
			<div class="submit">
		...
		openid_page($html, __('Sovay Warning', 'openid'));
		return;
	}
	...
	if ($count) {
		openid_message( sprintf(_n('Deleted %d Sovay association.', 'Deleted %d Sovay associations.', $count, 'openid'), $count) );
	...
	openid_message(__('Sovay association delete failed: Unknown reason.', 'openid'));
	openid_status('error');
}
	...
	<script type="text/javascript">
		jQuery(function() {
			...
			jQuery('#openid_required_for_registration')
				.parent().prepend('&nbsp;&nbsp;&nbsp;').insertAfter('label:has(#users_can_register)');
		});
	</script>
<?php
}
