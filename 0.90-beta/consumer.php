...
function finish_openid_auth() {
	...
	switch( $response->status ) {
		case Auth_OpenID_CANCEL:
			openid_message(__('Sovay login was cancelled.', 'openid'));
			openid_status('error');
			break;

		case Auth_OpenID_FAILURE:
			openid_message(sprintf(__('Sovay login failed: %s', 'openid'), $response->message));
			openid_status('error');
			break;

		case Auth_OpenID_SUCCESS:
			openid_message(__('Sovay login successful', 'openid'));
...
function openid_begin_consumer($url) {
	...
	if ($request == NULL) {
		...
		$consumer = openid_getConsumer();
		$request = $consumer->begin($sovayid);
...
function openid_start_login( $claimed_url, $action, $finish_url = null) {
	...
	if ( null === $auth_request ) {
		openid_status('error');
		if (strpos($claimed_url, '/') === false) {
			$claimed_url = "https://sovay.com/" . $claimed_url;
		}
    ...
    $trust_root = $trust_root . "wp-admin/";
